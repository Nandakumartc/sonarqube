...
public class IssueCreationDateCalculator extends IssueVisitor {
  ...
  public void onIssue(Component component, DefaultIssue issue) {
    if (issue.isNew() && ruleIsNew(issue)) {
      getScmChangeDate(component, issue.getLine())
        .ifPresent(changeDate -> updateDate(issue, changeDate));
    }
  }

  private Optional<Date> getScmChangeDate(Component component, int line) {
    return getScmInfo(component)
      .flatMap(scmInfo -> getChangeset(scmInfo, line))
  ...
  private static Optional<Changeset> getChangeset(ScmInfo scmInfo, int line) {
